...
import { isValidEmail, PRESET_AMOUNTS } from '@/utils';
import { DonationService, PaymentMethod } from '../services/index.service';
...
export default function DonatePage() {
  ...
  const priorityCheck = () => {
    ...
    if (
      isTaxDeductible === 'yes' &&
      (!donorName.trim() || !donorEmail.trim())
    ) {
      setSubmitMessage(
  ...
  const makeBitcoinDonation = async () => {
    ...
    try {
      const donationId = DonationService.generateId({
        amountUsd: amount as number,
        paymentMethod: PaymentMethod.BITCOIN,
      });

      // First, create donation record with pending status
      const createRecord = await DonationService.createRecord({
        donationId: donationId,
        name: donorName.trim(),
        email: donorEmail.trim(),
        amountUsd: amount as number,
        isTaxDeductible: isTaxDeductible === 'yes' ? true : false,
        paymentMethod: PaymentMethod.BITCOIN,
      });

      if (!createRecord.success) {
        if (createRecord instanceof Error) {
          const errorMsg = `Error creating donation record: ${createRecord.message}`;
          setSubmitMessage(errorMsg);
          trackDonationFailure('bitcoin', errorMsg);
          return;
        }
      }
      const randomDescription = DonationService.getRandomDonationDescription();
      ...
      if (response.ok && data.success) {
        ...
        setSubmitMessage(
          'Redirecting to Bitcoin payment page. The donation will be tracked automatically when payment is received.'
        );
      } else {
  ...
  const makeStripeDonation = async () => {
    ...
    try {
      ...
      // Then create donation record in database with pending status
      const createRecord = await DonationService.createRecord({
        donationId: donationId,
        name: donorName.trim(),
        email: donorEmail.trim(),
          ...
          <div className="max-w-3xl mx-auto space-y-6 sm:space-y-8">
            <div className="space-y-3 text-center">
              ...
              <p className="mt-1 text-[#090909] text-sm sm:text-base">
                Your contribution supports freedom tech builders across the
                globe. We're talking: less burnout, more impact. Executive
                coaching cultivates independent thought, which is exactly what
                we need for this human-powered revolution. <br /> <br />{' '}
                Together, let's strengthen the human infrastructure of open
                source. <br /> <br /> Thank you 🙏
              </p>
            </div>
            ...
            <div className="text-center space-y-3">
              <p className="mt-1 text-[#090909] text-sm sm:text-base">
                The OS Waye Foundation (EIN: 99-5041645) is a 501(c)(3)
                non-profit organization. All donations are tax-deductible to the
                full extent of the law.
              </p>
              <p className="text-[#090909] text-sm sm:text-base">
                You can donate fiat or anonymously with bitcoin. For a tax
                deduction, provide your name and email so we can send you a
            ...
            <div className="space-y-5 sm:space-y-6">
              <div className="text-center">
                <p className="mb-3 sm:mb-2 text-sm sm:text-base">
                  Do you want this donation to be tax deductible?
                </p>
              ...
              <div className="grid gap-4 sm:gap-4 md:grid-cols-2">
                <div className="flex flex-col gap-2">
                  <label className="text-sm sm:text-base">
                    Name{' '}
                    {!selectedOptionTaxDeductible && <span>(optional)</span>}
                  </label>
                  <input
                    type="text"
              ...
              <div className="text-center">
                <p className="mb-3 sm:mb-4 text-sm sm:text-base font-medium">
                ...
                <div className="flex flex-wrap gap-2 sm:gap-3 justify-center">
                  ...
                  <div className="relative w-full sm:flex-1 sm:max-w-[200px]">
                    <span className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 text-sm sm:text-base">
                      $
                    </span>
                    <input
                      type="text"
            ...
            <div className="text-center mt-12 sm:mt-16 mb-12 sm:mb-16">
              <p className="text-[#090909] text-sm sm:text-base">
                If you are interested in setting up a recurring donation, using
                a wire transfer, donating via donor-advised fund or have any
                questions please reach out to{' '}
                <a
                  href="mailto:hello@waye.dev"
                  className="text-blue-custom-100 hover:underline"
                >
                  hello@waye.dev
                </a>
                .
